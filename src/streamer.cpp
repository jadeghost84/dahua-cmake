...
#define STREAM_PIX_FMT AV_PIX_FMT_YUV420P
    ...
    int Streamer::init(const StreamerConfig &streamer_config)
    {
        ...
        if (avformat_alloc_output_context2(&format_ctx, nullptr, "rtsp", config.server.c_str()) < 0)
        {
            printf("Fail: avformat_alloc_output_context2\n");
            return false;
        }
        std::cout << "-----------------------------------------" << format_ctx->oformat->flags << std::endl;
        if (!format_ctx)
        {
            return 1;
        }
        av_opt_set(format_ctx->priv_data, "rtsp_transport", "tcp", 0);
        // AVIOContext for accessing the resource indicated by url
        std::cout << "-----------------------------------------" << format_ctx->oformat->flags << std::endl;
        if (!(format_ctx->oformat->flags & AVFMT_NOFILE))
        {
            int avopen_ret = avio_open2(&format_ctx->pb, format_ctx->url,
